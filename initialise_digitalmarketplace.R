...
master_descriptions = map_df(pages, function(j){
  ...
  descriptions <- map_dfr(result_entries,function(i){
    ...
    titles_data<- i %>% html_nodes('h2') %>% html_text()
    link <- i %>%html_nodes('h2') %>% html_nodes('a') %>% html_attr('href')%>%unlist()
    link <- paste0(main,link)
    content_data = i %>% html_nodes('p')%>% html_text()
...
master_descriptions <-master_descriptions %>% mutate('interesting' = ifelse(master_descriptions$Rating < 1, F, T)) %>% mutate('sent' = FALSE) %>% distinct(title, .keep_all = TRUE)
