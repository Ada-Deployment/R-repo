...
descriptions <- map_dfr(1:10, function(i) {
  ...
  title_data <- html_nodes(pg, "div.search-result-header") %>%
    html_attr("title") %>%
    unlist()

  # extract project further info url
  link_data <- html_nodes(pg, "div.search-result-header") %>%
    html_nodes("a") %>%
    html_attr("href") %>%
    unlist()

  # extract project id
  id_data <- html_nodes(pg, "div.search-result-header") %>%
    html_nodes("h2") %>%
    html_attr("id") %>%
    unlist()

  # extract project abstract/description
  desc_data <- html_nodes(pg, "div.wrap-text") %>%
    html_text() %>%
    unlist()
  ...
  content_data <- map_dfr(result_entries, function(contract) {
    titles <- contract %>%
      html_nodes("div.search-result-entry") %>%
      html_nodes("strong") %>%
      html_text()
    content <- contract %>%
      html_nodes("div.search-result-entry") %>%
...
descriptions <- descriptions %>%
  left_join(., master_descriptions %>% select(id, sent)) %>%
  mutate(sent = ifelse(is.na(sent), F, sent)) %>%
