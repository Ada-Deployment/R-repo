...
smtp <- emayili::server(host = "email-smtp.eu-west-1.amazonaws.com",
               port = 587,
...
for(file in portal){
  ...
  relevant_contracts = master_descriptions %>% filter(interesting == TRUE, sent==FALSE)
  print(nrow(relevant_contracts))
  master_descriptions = master_descriptions %>% filter(interesting == TRUE) %>% mutate(sent=ifelse(title %in%  relevant_contracts$title, TRUE, sent))
